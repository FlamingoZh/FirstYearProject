...
def run_experiments(fp_base, is_demo):
    ...
    do_correlation_word_audio = False
    do_correlation_image_audio = False
    do_strength_word_image = False
    do_strength_word_audio = False
    do_strength_image_audio = False
    do_strength_word_image_audio = False
    do_strength_noise = False
    do_strength_word_image_aoa = False
    do_strength_word_imagenet = False

    # Figures.
    # Set a variable to False if you would like to skip remaking the figure.
    do_plot_fig_3 = True
    do_plot_fig_4 = False
    do_plot_fig_s1 = False
    do_plot_fig_s2 = False
    ...
    if do_correlation_word_image:
        # Load intersection.
        print("fp_intersect_word_image:",fp_intersect_word_image)
        print("fp_corr_word_image:",fp_corr_word_image)
        #intersect_data = pickle.load(open(fp_intersect_word_image, 'rb'))
        intersect_data = pickle.load(open("../my_intersection_image_word_vrd.pkl", 'rb'))
        ...
        # ]
        #set_size_list = np.array([10, 30, 98], dtype=int)
        set_size_list = np.array([10, 20, 30, 50, 98], dtype=int)
        sub_list = [
            np.array([3, 4, 5, 6, 7, 8, 9, 10], dtype=int),
            np.arange(3, 21, 1, dtype=int),
            np.arange(3, 31, 1, dtype=int),
            np.arange(3, 51, 1, dtype=int),
            np.arange(3, 99, 1, dtype=int)
        ]
    ...
    if do_plot_fig_3:
        ...
        print("---------------------")
        print("set_size_list[idx]:",set_size_list[idx])
        print("n_mismatch_list[idx]:",len(n_mismatch_list[idx]),n_mismatch_list[idx])
        print("rho_list[idx]:",len(rho_list[idx]),rho_list[idx])
...
def run_primary_analysis(
    ...
    z_1 = z_list[1]
    n_item = z_0.shape[0]
    print("all:",z_0.shape,z_1.shape,n_item)
    ...
    for set_idx, set_size in enumerate(set_size_list):
        ...
        #print("sub:",z_0_sub.shape,z_1_sub.shape)
        ...
        for val_sub in sub_list[set_idx]:
            ...
            #print("n_mismatch_uniq:",n_mismatch_uniq)
...
def permutation_analysis_last(sim_mat_list, max_perm=10000, n_known=0):
    ...
    for perm_idx in range(n_perm):
        ...
        for sim_mat_idx in range(n_sim_mat - 1):
            # print("!!!!!!!!!!!!")
            # print(perm_list_all[sim_mat_idx][perm_idx + 1, :])
            # print("----------------")
            # print(sim_mat_list[sim_mat_idx + 1])
            # print("***********************")
            # print(symmetric_matrix_indexing(sim_mat_list[sim_mat_idx + 1], perm_list_all[sim_mat_idx][perm_idx + 1, :]))
